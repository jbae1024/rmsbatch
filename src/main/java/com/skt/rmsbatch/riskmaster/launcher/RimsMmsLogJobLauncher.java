 ...
 */
@Component
public class RimsMmsLogJobLauncher {

    private static final Logger LOGGER = LoggerFactory.getLogger(RimsMmsLogJobLauncher.class);
    ...
    @Autowired
    RimsMmsLogJobLauncher(@Qualifier("rimsMmsLogJob") Job job, JobLauncher jobLauncher) {
        this.job = job;
        this.jobLauncher = jobLauncher;
    }

    @Scheduled(cron = "${hioms.rimsMmsLogJob.cron}")
    ...
    void launchXmlFileToDatabaseJob() throws JobParametersInvalidException, JobExecutionAlreadyRunningException, JobRestartException, JobInstanceAlreadyCompleteException {
        LOGGER.info("Starting rimsMmsLogJob job");

        jobLauncher.run(job, newExecution());

        LOGGER.info("Stopping rimsMmsLogJob job");
    }
